...
os.chdir("C://projects//election_forecast//election_map")

# Import Elections Canada geojson
gdf = geopandas.read_file('electoral_districts_2022_fed.geojson')
...
probs = pd.read_csv("C://projects//election_forecast//model_results//ridingprobabilities.csv")
...
gdf = gdf.rename(columns = {'lpc':'LPCwins', 'cpc':'CPCwins', 'ndp':'NDPwins', 'gpc':'GPCwins', 'bq':'BQwins', 'ppc':'PPCwins'})
...
gdf['PPCwins'] = pd.to_numeric(gdf['PPCwins'])

# Create columns for winning Party and probability
gdf['Winner'] = ''
gdf['Margin'] = ''
margin = gdf[['LPCwins', 'CPCwins', 'NDPwins', 'GPCwins', 'BQwins', 'PPCwins']]
...
def func_colour_index(x):
    '''Asign colour to riding based on most probability of outcome'''
    ...
    if (x['Winner'] == 'CPCwins') and (x['Margin'] <= 40):
        ...
        return '#ffffff'
gdf['Fill'] = gdf.apply(func_colour_index, axis = 1)
# Drop the Winner and Margin columns
gdf = gdf.drop(columns = ['Winner', 'Margin', 'LPCwins', 'CPCwins', 'NDPwins', 'GPCwins', 'BQwins', 'PPCwins'])

# Read in riding vote shares
probs = pd.read_csv("C://projects//election_forecast//model_results//ridingvotepercentages.csv")
# Rename ridingid to FEDNUM for merge
...
gdf = geopandas.GeoDataFrame(df)
# Capitalize Party names
gdf = gdf.rename(columns = {'lpc':'LPC', 'cpc':'CPC', 'ndp':'NDP', 'gpc':'GPC', 'bq':'BQ', 'ppc':'PPC'})
